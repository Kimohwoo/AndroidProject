...
//        val file = File(filesDir, filename)
        ...
        val calorie = String.format("%.1f", 0.3 * exerciseTimeSeconds)

        if (totalDistance != null) {
            binding.progressbar.progress = totalDistance
            binding.totalDistance.text = String.format("%.1fkm", totalDistance / 1000.0)
            binding.totalDistance.setTextSize(TypedValue.COMPLEX_UNIT_PT, 120f) // 글자 크기를 18sp로 설정
            binding.achivement.text = "산책량 ${(totalDistance / 35).toInt()}%"
            binding.achivement.setTextSize(TypedValue.COMPLEX_UNIT_PT, 60f) // 글자 크기를 18sp로 설정

        }

        binding.calorie.text = calorie + "kal"
        binding.calorie.setTextSize(TypedValue.COMPLEX_UNIT_PT, 120f) // 글자 크기를 18sp로 설정
        ...
        Log.d(
            "ksj12",
            "총거리 : $totalDistance m, 파일이름 : $fileName, 파일경로 : $filePath, 운동시간 : $formattedExerciseTime"
        )
        Log.d("ksj12", "$dayNum")
        ...
        if (file.exists()) {
            ...
            if (lines.isNotEmpty()) {
                ...
                val firstLatLng = LatLng(
                    lines.first().split(",")[0].trim().substringAfter(":").trim().toDouble(),
                    lines.first().split(",")[1].trim().substringAfter(":").trim().toDouble()
                )
...
//                MarkerOptions().position(lastLatLng)를 사용하여 마커를 생성하고, 이를 mMap의 addMarker() 메서드로 추가합니다. 이렇게 하면 마지막 위치에 해당하는 좌표에 마커가 표시됩니다.
                val lastLatLng = LatLng(
                    lines.last().split(",")[0].trim().substringAfter(":").trim().toDouble(),
                    lines.last().split(",")[1].trim().substringAfter(":").trim().toDouble()
                )
    ...
    private fun displayPathFromTextFile2() {
        ...
        if (totalDistance != null) {
            binding.progressbar.progress = totalDistance
            binding.totalDistance.text = String.format("%.1fkm", totalDistance)
            binding.totalDistance.setTextSize(TypedValue.COMPLEX_UNIT_PT, 120f) // 글자 크기를 18sp로 설정
            binding.achivement.text = "산책량 ${(totalDistance / 35).toInt()}%"
            binding.achivement.setTextSize(TypedValue.COMPLEX_UNIT_PT, 60f) // 글자 크기를 18sp로 설정
